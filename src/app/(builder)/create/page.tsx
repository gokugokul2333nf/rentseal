...
import { TemplateThumb } from "@/components/landing/template-thumb";
...
const CATEGORY_BLURB: Record<AgreementTemplate["category"], string> = {
  Residential:
    "Somewhere to live — let on monthly rent, usually for eleven months.",
  Commercial:
    "Trade premises. Longer terms, and the business clauses come with them.",
  "Lease deed":
    "Twelve months and over, registrable, with the strongest evidentiary position.",
  "Leave & licence":
    "Permission to occupy without granting a tenancy, so possession comes back cleanly.",
  ...
  "Deeds & undertakings":
    "One-off documents: an indemnity, an undertaking, a private loan.",
  Affidavits:
    "Sworn declarations — property, a lost document, a study gap, consent for a child to travel.",
  "Tamil — தமிழ்":
      ...
      <div className="container-page py-16 md:py-24">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <Reveal delay={0.12}>
            <p className="mt-5 text-[17px] leading-[1.7] text-navy-600">
              Pick the one closest to your situation and its clauses, its term
              and its wording come with it. If you get it wrong you can switch
              at any point — we keep the parties and the address and re-draft
              the rest.
            </p>
          </Reveal>
        </div>
                  ...
                  return (
                    <StaggerItem key={template.id} className="h-full">
                      <Link
                        ...
                        className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-line bg-white shadow-soft transition-all duration-300 hover:-translate-y-1 hover:border-brand-500 hover:shadow-lift"
                      >
                        ...
                        <div className="flex flex-1 flex-col p-5">
                          <h2 className="relative font-display text-[15.5px] leading-snug font-bold text-navy-950">
                            {template.name}
                          </h2>
                          ...
                          {spec ? (
                            <p className="relative mt-3 text-[12px] text-navy-400">
                              ...
                              · {spec.roleA.toLowerCase()} and{" "}
                              {spec.roleB.toLowerCase()}
                            </p>
                          ) : null}
        ...
        <Reveal delay={0.26}>
          <p className="mt-6 text-center text-[13px] text-navy-400">
            ...
            Not sure which one? Start with the closest and change it inside the
            builder.
          </p>
        </Reveal>
      </div>
